  ...
  return (
    ...
    >
      <div
        ...
        dir={dir}
        className={cn(
          "flex items-center flex-wrap gap-1 p-2 border rounded-lg overflow-hidden",
          {
      ...
      >
        {value?.map((item, index) => (
          <Badge
            ...
            className={cn(
              "relative p-2 text-[.95rem] focus:!ring-0 focus:ring-offset-0 focus:!outline-0 rounded flex items-center gap-1 data-[active='true']:ring-2 data-[active='true']:ring-muted-foreground truncate aria-disabled:opacity-50 aria-disabled:cursor-not-allowed",
              badgeClassName
            )}
            variant={"secondary"}
          >
        ...
        <Input
          ...
          onBlur={(e) => {
            if (changeOnBlur) {
              activeIndex === -1 ? handleBlur(e) : undefined;
            }
          }}
          className={cn(
            "outline-0 border-none py-4 min-w-fit text-[.95rem] ring-0 shadow-none flex-1 focus-visible:outline-0 focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-0 placeholder:text-muted-foreground ",
