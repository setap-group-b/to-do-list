...
import { BookOpen, Calendar, Clock, GraduationCap, ListChecks, Sparkles, Trophy } from "lucide-react";
...
export default async function Home() {
  ...
  const upcomingDeadlines = [
    { id: 1, title: "React Project", course: "SETAP 101", dueDate: "Tomorrow, 11:59 PM", priority: "high" },
    { id: 2, title: "Usability CW", course: "USAB 202", dueDate: "Friday, 5:00 PM", priority: "medium" },
    { id: 3, title: "Login System", course: "COMP 210", dueDate: "Next Monday", priority: "low" },
  ];
  
  const studySessions = [
    { id: 1, subject: "SETAP", duration: "2 hours", time: "Today, 3:00 PM", location: "Library, Floor 2" },
    { id: 2, subject: "Web Programming", duration: "1 hour", time: "Tomorrow, 4:30 PM", location: "Student Center" },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold mb-2">
              Good {timeOfDay}, {session?.user?.name}! <Sparkles className="inline-block h-6 w-6 ml-1" />
            </h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-indigo-100 dark:border-indigo-900/40 overflow-hidden">
          <div className="flex items-center justify-between bg-indigo-50 dark:bg-indigo-900/30 p-4 border-b border-indigo-100 dark:border-indigo-800/30">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-indigo-900 dark:text-indigo-200">Upcoming Deadlines</h2>
            </div>
            <button className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium">View All</button>
          </div>
          <div className="p-4">
            <ul className="space-y-3">
              {upcomingDeadlines.map((task) => (
                <li key={task.id} className="flex items-center gap-3 p-3 rounded-xl bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/30">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                      <span className="bg-indigo-100 dark:bg-indigo-800/50 px-2 py-0.5 rounded-full">{task.course}</span>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-indigo-100 dark:border-indigo-900/40 overflow-hidden">
          <div className="flex items-center justify-between bg-indigo-50 dark:bg-indigo-900/30 p-4 border-b border-indigo-100 dark:border-indigo-800/30">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-indigo-900 dark:text-indigo-200">Study Sessions</h2>
            </div>
            <button className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium">Schedule</button>
          </div>
          <div className="p-4">
            <ul className="space-y-3">
              {studySessions.map((session) => (
                <li key={session.id} className="flex items-center gap-3 p-3 rounded-xl bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/30">
                  ...
                  <div className="flex-1">
                    <h3 className="font-medium text-gray-900 dark:text-gray-100">{session.subject}</h3>
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-indigo-500/10 dark:from-purple-900/20 dark:to-indigo-900/20 p-5 rounded-2xl border border-indigo-200/50 dark:border-indigo-800/30 text-center">
        ...
        <p className="text-indigo-600 dark:text-indigo-400 text-sm mt-1">— B.B. King</p>
      </div>
    </div>
  );
}
